...
function parseTodo(line: string): Todo | undefined {
  //@ts-ignore
  const structRegex =
    /^(?<done>[xX]?\ *)(?:\((?<priority>[A-Z])\)\ )?(?<body>.+)\ ?/gi;

  const contextRegex = /\@\w+/gim;
  const tagsRegex = /\+\w+/gim;
  //@ts-ignore
  const attributesRegex = /(?<key>\w+)\:(?<value>[\w\-\_]+)/gim;

  const parsedTodo = structRegex.exec(line)?.groups;

  if (!parsedTodo) return undefined;

  const contexts = parsedTodo.body
    .matchAll(contextRegex)
    .toArray()
    .map((a) => a[0].substring(1));
  const tags = parsedTodo.body
    .matchAll(tagsRegex)
    .toArray()
    .map((a) => a[0].substring(1));
  const attributes = parsedTodo.body
    .matchAll(attributesRegex)
    .toArray()
    .map((a) => a.groups) as { key: string; value: string }[];
...
function serialiseTodo(todo: Todo) {
  ...
  }${todo.body}${todo.attributes.map((a) => ` ${a.key}:${a.value}`).join("")}`;
}
...
function dumpToDone(todos: Todo[]) {
  ...
  const fileContents =
    todos.map((t) => serialiseTodo(t)).join("\n") + "\n" + done;
  writeFileSync(donePath, fileContents);
}

export const TodoRow: FC<{ todo: Todo; readOnly?: boolean }> = ({
  todo,
  readOnly,
}) => {
    ...
    >
      ...
      <td style={"width: 3rem"}>
        <select
          name="newPriority"
          hx-vals={vals}
          hx-post="/set-priority"
          hx-target="body"
        >
          <option value="">-</option>
          {"ABCDEFGHIJKLMNOP".split("").map((char) => (
            <option value={char} selected={char == todo?.priority}>
              {char}
            </option>
          ))}
        </select>
      ...
      <td style={"width: 3rem"}>
        ...
        >
          <option value="">-</option>
          {"SML".split("").map((char) => (
            <option value={char} selected={size == char}>
              {char}
            </option>
          ))}
        </select>
      ...
      <td>
        <textarea
          style={
            ...
            (todo.isDone && !readOnly ? "text-decoration: line-through" : "")
          }
          hx-trigger="change"
          hx-post="/update-body"
          hx-swap="outerHTML"
      ...
      {figmaLink ? (
        ...
        >
          <a
            ...
            href={
              "https://www.figma.com/design/zNVxc3pmP7T96YecpR6TJS/Busch-Protective?node-id=" +
              figmaLink
            }
            target="_blank"
          >
            Open <i class="bi bi-link"></i>
          </a>
        </td>
      ) : (
        <td style={"width: 8rem"}>
...
app.post("/set-priority", async (c) => {
  ...
  const nextTodos = todos.map((t) => {
    if (t.body == body.body && t.priority == body.priority)
      return {
        ...t,
        priority: body.newPriority as string,
      };
    return t;
  });
  todos = nextTodos;
  ...
  const nextTodos = todos.map((t) => {
    if (t.body == body.body && t.priority == body.priority) {
      nextTodo = {
        ...t,
        attributes: [
          ...t.attributes.filter((a) => a.key != "size"),
          { key: "size", value: newSize },
        ],
      };
      assert(nextTodo);
      return nextTodo;
  ...
  const nextTodos = todos.map((t) => {
    if (t.body == body.body && t.priority == body.priority) {
      nextTodo = {
        ...t,
        attributes: [
          ...t.attributes.filter((a) => a.key != "figma"),
          { key: "figma", value: figmaNodeId },
        ],
      };
      assert(nextTodo);
      return nextTodo;
...
app.post("/toggle-done", async (c) => {
  ...
  const nextTodos = todos.map((t) => {
    if (t.body == body.body && t.priority == body.priority) {
      nextTodo = {
        ...t,
        isDone: body.shouldBeDone ? true : false,
      };
      return nextTodo;
    }
    return t;
...
app.get("/", (c) => {
  ...
  const showDone = c.req.query("showDone");

  const searchFilter = c.req.query("q");

  const smallTodos = todos.filter((to) =>
    to.attributes.some((a) => a.key == "size" && a.value == "S")
  ).length;
  const mediumTodos = todos.filter((to) =>
    to.attributes.some((a) => a.key == "size" && a.value == "M")
  ).length;
  const largeTodos = todos.filter((to) =>
    to.attributes.some((a) => a.key == "size" && a.value == "L")
  ).length;
  ...
  return c.html(
    <html>
      ...
      <body>
        ...
        <script
          src="https://cdn.jsdelivr.net/npm/@material/ripple@14.0.0/dist/mdc.ripple.min.js"
          integrity="sha384-9QANVmWxL3S8VRs8x1Q+bF1Zzogpy7P/Qw1+y5qHLdC1ig0EuoHg9VbB1SXyecdZ"
          crossorigin="anonymous"
        ></script>

        <script
          src="https://cdn.jsdelivr.net/npm/@materialstyle/materialstyle@3.1.1/dist/js/materialstyle.min.js"
